...
**方式 B（v0.1 で実装済み・メンバーごとに任意）: Google Calendar API による即時同期**
- メンバーが `/me` の「Google と連携する」から OAuth 認可すると、本システムが本人のカレンダーへ召集予定を直接作成・更新・削除する。反映は即時。中止・不参加回答・召集解除で即時に消える
- 加えて **空き時間の自動取り込み**（FreeBusy 読み取り）を行い、カレンダーの「予定あり」を可用性の「不可」として登録する（毎朝更新＋手動取り込み。予定の内容は読まない。本システムが登録した予定は透過扱いにして二重計上しない）
- リフレッシュトークンは AES-256-GCM で暗号化して保存。連携解除時は登録した予定を削除し、トークンを失効させる
- 運用開始には Google Cloud プロジェクトの OAuth クライアント作成と、同意画面の審査（`calendar.events` は機微スコープ。審査前はテストユーザー 100 名まで）が必要
- 方式 A（iCal）は方式 B を使わないメンバー向けのフォールバックとして常に提供する
...
詳細は `supabase/migrations/20260913014122_rehearsal_management.sql` を正とする。
...
| Q7 | **Google カレンダー連携の方式**。iCal 購読は設定が簡単だが反映が遅い（Google 側で数時間〜1 日）。API 連携は即時だが Google Cloud の OAuth 審査と各人の認可操作が必要 | (a) iCal のみ (b) iCal + API（希望者のみ API） (c) API のみ | (b) を実装済み。**Google Cloud の OAuth 同意画面審査を誰が進めるか**、審査完了までテストユーザー登録で運用するかを決めたい。急な変更は LINE 通知でも補う |
...
| フェーズ | 内容 |
|---|---|
| **v0.1（本ブランチで実装）** | プロダクション／メンバー／シーン管理、稽古枠の作成・召集・出欠・実施シーン記録、シーン進捗、可用性登録とマトリクス、作成前の参加可否チェック、本番シフト、代役募集（先着確定）、個人ビュー、iCal フィード、Google Calendar API 同期＋FreeBusy 取り込み、LINE 連携（連携・プッシュ・問い合わせ・毎朝配信・前日リマインド） |
| v0.2 | 可用性の繰り返し設定、シーンのタグ（ルート）、会場マスタ、シフト CSV 取り込み、出演回数の契約ビュー |
...
| `/api/google/connect` `/api/google/callback` | Google | OAuth 認可の開始・コールバック |
| `/api/line/webhook` | LINE | Messaging API Webhook |
| `/api/cron/rehearsal-notify` | スケジューラ | 毎朝配信・前日リマインド（外部スケジューラから 10〜30 分間隔で呼ぶ） |

環境変数（`.env.example` 参照）: `LINE_CHANNEL_SECRET` / `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_ADD_FRIEND_URL` / `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_TOKEN_KEY` / `REHEARSAL_DIGEST_HOUR` / `REHEARSAL_REMINDER_HOUR`。LINE・Google が未設定でも画面上の機能と iCal は動作する。

DB マイグレーションは Supabase プロジェクト `wiqnmebudaadwqdaxwko` に適用済み（`rehearsal_management`, `rehearsal_google_calendar`）。
